...
class AgvModel
  ...
  void init() {
    if (simulator.isPresent() && rm.isPresent()) {
      ...
      for (int i = 0; i < num; i++) {
        ...
        final factory.Box b = new factory.Box(rnd, dest, duration, bh);
        bh.box = b;
  ...
  @Override
  public boolean register(factory.AGV element) {
    element.registerAgvModel(this);
    return true;
  }

  @Override
  public boolean unregister(factory.AGV element) {
    return false;
  }
  ...
  public void handleEvent(Event e) {
    ...
    final factory.Box box = (Box) verifyNotNull(event.parcel);
    ...
    if (e.getEventType() == PDPModelEventType.END_DELIVERY) {
      ...
      final factory.Box newBox = new factory.Box(box.getDeliveryLocation(),
        dest, duration, bh);
      bh.box = newBox;
  ...
  @Nullable
  factory.Box nextDestination() {
    if (boxes.isEmpty()) {
      init();
    }
    verify(!boxes.isEmpty());
    final factory.Box b = boxes.get(currentBox % boxes.size()).box;
    currentBox++;
    return b;
  }
  ...
  @AutoValue
  abstract static class Builder extends AbstractModelBuilder<AgvModel, factory.AGV> {
