...
import factory.AGV;
import factory.AgvModel;
import factory.BoxRenderer;
...
public final class FactoryExample {
  private static final Logger LOGGER = LoggerFactory.getLogger(FactoryExample.class);

  static final double POINT_DISTANCE = 5d;
  static final long SERVICE_DURATION = 120000;
  static final double AGV_SPEED = 2;
  ...
  public static Simulator run(final long endTime, Display display,
    ...
    final List<String> words = asList("OMG\nLOL");
   ...
   final ImmutableList<ImmutableList<Point>> points = createPoints(words);
    ...
    View.Builder view = View.builder()
      .with(//GraphRoadModelRenderer.builder()
            WarehouseRenderer.builder().withNodes().withOneWayStreetArrows()
        .withMargin(CANVAS_MARGIN))
      .with(BoxRenderer.builder())
      .with(
        RoadUserRenderer.builder()
          .withImageAssociation(AGV.class,
            "/graphics/flat/forklift2.png").withCircleAroundObjects())
    ...
    final ListenableGraph<LengthData> test = simple.SimpleExample.GraphCreator.createGraph();
    final RandomGenerator rng = new MersenneTwister(RANDOM_SEED);
    final Simulator simulator = Simulator
      .builder()
      .setRandomGenerator(rng)
      .addModel(RoadModelBuilders.dynamicGraph(simple.SimpleExample.GraphCreator.createGraph())
    ...
    for (int i = 0; i < NUM_VEHICLES; i++) {
      ...
      final Point p = test.getRandomNode(rng);
      try {
        simulator.register(new AGV(p));
      }catch(IllegalArgumentException e){
        LOGGER.warn("Could not register AGV at pos " + p);
      }
    }
